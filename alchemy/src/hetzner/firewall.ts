...
export const Firewall = Resource(
  ...
  async function (
      ...
      if (this.output?.id) {
        try {
          ...
          if (this.output.appliedTo && this.output.appliedTo.length > 0) {
            try {
              await api.post(
                `/firewalls/${this.output.id}/actions/remove_from_resources`,
                {
              ...
              await withExponentialBackoff(
                async () => {
                  const { firewall } = await api.get<{ firewall: any }>(
                    `/firewalls/${this.output!.id}`,
                  );
    ...
    const targetResources: HetznerFirewallResource[] = (
      props.applyTo ?? []
    ).map((target) => {
    ...
    if (this.phase === "create" || !firewallId) {
      if (props.adopt && !this.isReplacement) {
        try {
          const { firewalls } = await api.get<{ firewalls: any[] }>(
            `/firewalls?name=${name}`,
          );
          ...
          await withExponentialBackoff(
            ...
            (error) => {
              return error.message?.includes(
                "no public network interfaces found",
              );
            },
            10,
            2000,
      ...
      if (
        ...
        const response = await api.put<{ firewall: any }>(
      ...
      if (currentTargetsStr !== newTargetsStr) {
        ...
        // Calculate diff
        const toRemove = this.output.appliedTo.filter(
          (curr) =>
            !targetResources.some(
              (next) => JSON.stringify(next) === JSON.stringify(curr),
            ),
        );
        const toAdd = targetResources.filter(
          (next) =>
            !this.output.appliedTo.some(
              (curr) => JSON.stringify(curr) === JSON.stringify(next),
            ),
        );
      ...
      const response = await api.get<{ firewall: any }>(
        `/firewalls/${firewallId}`,
      );
      firewallData = response.firewall;
    }
 ...
 */
export function isFirewall(resource: unknown): resource is Firewall {
  return (resource as any)?.[ResourceKind] === "hetzner::Firewall";
}
